...
case $WGET_OUTPUT in
        "200")
                #URL is responding
                if [[ $REVERSE ]]; then
                        FINAL_COMMENT="CRITICAL: URL is available and it shouldn't - $WGET_OUTPUT"
                        FINAL_STATE=$CRITICAL
                else
                        FINAL_COMMENT="OK: URL is available and it should - $WGET_OUTPUT"
                        FINAL_STATE=$OK
                fi
                ;;
        "403" | "503")
                ...
                if [[ $REVERSE ]]; then
                        FINAL_COMMENT="OK: URL isn't available and it shouldn't - $WGET_OUTPUT"
                        FINAL_STATE=$OK
                else
                        FINAL_COMMENT="CRITICAL: URL isn't available and it should - $WGET_OUTPUT"
                        FINAL_STATE=$CRITICAL
                fi
                ;;
        "407")
